...
import { ArrowLeft, Search, Sparkles, CheckCircle2, XCircle, Shuffle, BookOpen } from 'lucide-react';
...
export default function RhetoricalDevicesPage() {
  const [tab, setTab] = useState<'katalog' | 'training'>('katalog');
  ...
  const quizItems = useMemo(() => {
    const base = devices.slice(0, 12);
    return base.map((device) => {
      const wrongOptions = shuffle(
        devices
          .filter((candidate) => candidate.id !== device.id)
          .map((candidate) => candidate.wirkung),
      ).slice(0, 3);

      const options = shuffle([device.wirkung, ...wrongOptions]);
      return {
        id: device.id,
        prompt: `Welche Wirkung passt am besten zu „${device.name}“?`,
        options,
        correctIndex: options.findIndex((option) => option === device.wirkung),
        explanation: `${device.name}: ${device.description}`,
      };
    });
  }, [devices]);
  ...
  const submitQuizAnswer = () => {
    if (selectedOption == null || showResult || !currentQuiz) return;
    ...
    <div className="min-h-screen flex flex-col bg-background selection:bg-primary/20">
      <main className="flex-1 container mx-auto px-4 pt-32 pb-24 max-w-7xl">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 px-2">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col gap-4 items-end">
            <div className="flex items-center gap-6 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/40">
              ...
              <div className="flex flex-col items-end">
                <span className="text-foreground">{quizItems.length}</span>
              ...
              {attempts > 0 && (
                <>
                  <div className="w-px h-6 bg-border/40" />
                  <div className="flex flex-col items-end">
                    <span className="text-foreground">{Math.round((score / attempts) * 100)}%</span>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          <Button variant={tab === 'katalog' ? 'default' : 'outline'} className="rounded-full" onClick={() => setTab('katalog')}>
            Katalog
          </Button>
          <Button variant={tab === 'training' ? 'default' : 'outline'} className="rounded-full" onClick={() => setTab('training')}>
            <Sparkles className="mr-2 h-4 w-4" /> Training
          </Button>
        </div>
            ...
            <Card className="card-modern border-border/50">
              <CardContent className="p-6 sm:p-8 space-y-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.2em] text-primary font-semibold">Training</p>
                    <h2 className="font-display text-2xl font-bold mt-2">Wirkung erkennen</h2>
                  </div>
                  <Button variant="outline" size="sm" className="rounded-full" onClick={resetTraining}>
                ...
                {currentQuiz ? (
                  <>
                    <div className="rounded-2xl border border-border/50 p-5">
                      <p className="text-sm text-muted-foreground mb-2">Frage {quizIndex + 1} von {quizItems.length}</p>
                      <p className="font-semibold text-lg">{currentQuiz.prompt}</p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      <Button className="rounded-full" onClick={submitQuizAnswer} disabled={selectedOption == null || showResult}>
                        Antwort prüfen
                      </Button>
                      <Button variant="outline" className="rounded-full" onClick={nextQuiz}>
                        Nächste Frage
                      </Button>
                    </div>

                    {showResult && (
                      <div className="rounded-2xl border border-border/50 p-4 bg-secondary/20">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="font-semibold">
                            {selectedOption === currentQuiz.correctIndex ? 'Richtig.' : 'Noch nicht korrekt.'}
                          </p>
                        </div>
                  ...
                  <p className="text-muted-foreground">Keine Übungen verfügbar.</p>
                )}

                <div className="rounded-2xl border border-border/50 bg-card p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground mb-2">Fortschritt</p>
                  <p className="text-sm">Punkte: {score} / {attempts}</p>
                </div>
              </CardContent>
            </Card>

            <Card className="card-modern border-border/50 mt-6">
              <CardContent className="p-5">
                <div className="flex items-center gap-2 mb-3 text-primary">
                  <BookOpen className="h-4 w-4" />
                  <p className="text-sm font-semibold">Lerntipp</p>
                </div>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Wechsle regelmäßig zwischen Katalog und Training. Erst Definition lesen, dann Wirkung aktiv abfragen - so verankerst du Stilmittel deutlich nachhaltiger.
                </p>
              </CardContent>
            </Card>
          </section>
