  ...
  useEffect(() => {
    if (authorId === 'seneca') {
      ...
      async function translateContent() {
        const translatedAuthor = await getTranslatedAuthor(language, 'seneca' as Author);
        setAuthorInfo(translatedAuthor);

        if (!postsLoading) {
          const translatedPosts = await Promise.all(
            allPosts.filter(p => p.author === 'seneca').slice(0, 3).map(p => getTranslatedPost(language, p.author, p.slug))
          );
