...
export function TermPopover({ term, children, type }: TermPopoverProps) {
  ...
  return (
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogContent 
        className="max-w-3xl w-[90vw] max-h-[85vh] p-0 gap-0 overflow-hidden rounded-lg"
        style={{ 
          position: 'fixed',
          left: '50%',
          top: '50%',
      ...
      >
        <DialogHeader className="p-4 sm:p-6 border-b border-border/50 bg-secondary/30 rounded-t-lg">
          <DialogTitle className="flex items-center gap-2">
            <BookOpen className="h-5 w-5 text-primary" />
            <span className="font-display">{term}</span>
        ...
        <ScrollArea className="h-[calc(85vh-180px)] p-4 sm:p-6">
          {loading ? (
