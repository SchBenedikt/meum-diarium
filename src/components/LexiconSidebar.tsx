...
export function LexiconSidebar({ entry }: LexiconSidebarProps) {
  ...
  return (
    <aside className="space-y-5 lg:space-y-6">
      ...
      {entry.etymology && (
        <motion.div
          className="rounded-2xl border border-border/40 bg-transparent p-5 lg:p-6 animate-in"
      ...
      {/* Category */}
      <motion.div
        className="rounded-2xl border border-border/40 bg-transparent p-5 lg:p-6 animate-in stagger-1"
        initial={{ opacity: 0, x: 12 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
      ...
      {relatedTerms.length > 0 && (
        <motion.div
          className="rounded-2xl border border-border/40 bg-transparent p-5 lg:p-6 animate-in stagger-2"
          initial={{ opacity: 0, x: 12 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
      ...
      {/* Go to Lexicon */}
      <motion.div
        className="rounded-2xl border border-border/40 bg-transparent p-5 lg:p-6 animate-in stagger-3"
        initial={{ opacity: 0, x: 12 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
