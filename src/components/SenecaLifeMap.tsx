...
const allMarkerPoints: MarkerPoint[] = [
  {
    title: 'Corduba',
    note: 'Geburtsort • 4 v. Chr.',
    description: 'Seneca wird in der Provinz Baetica (Spanien) als Sohn einer römischen Ritterfamilie geboren.',
    position: [37.88, -4.78], tone: 'birth',
  },
  {
    title: 'Rom',
    note: 'Politisches & literarisches Zentrum',
    description: 'Seneca kehrt früh nach Rom zurück, studiert Rhetorik und Philosophie und beginnt seine politische Karriere.',
    position: [41.90, 12.48], tone: 'core',
  },
  {
    title: 'Corsica',
    note: 'Exil auf Korsika • 41–49 n. Chr.',
    description: 'Von Claudius nach Korsika verbannt (angeblicher Ehebruch mit Iulia Livilla). Schreibt ›Trostschrift an seine Mutter Helvia‹.',
    position: [42.15, 9.05], tone: 'exile',
  },
  {
    title: 'Rom (Rückkehr)',
    note: 'Rückkehr & Macht • 49–62 n. Chr.',
    description: 'Von Agrippina zurückgerufen, wird Senecas Erzieher Neros und faktischer Mitregent.',
    position: [41.90, 12.48], tone: 'core',
  },
  {
    title: 'Ostia',
    note: 'Rückzug • 62–65 n. Chr.',
    description: 'Seneca zieht sich aus der Politik zurück, widmet sich der Philosophie: Epistulae Morales, Naturales Quaestiones.',
    position: [41.75, 12.28], tone: 'core',
  },
  {
    title: 'Rom (Tod)',
    note: 'Selbstmord • April 65 n. Chr.',
    description: 'Wegen der Pisonischen Verschwörung zum Tod verurteilt. Seneca öffnet sich die Adern – todkrank diktierte er bis zum Ende.',
...
const allRouteSegments: RouteSegment[] = [
  {
    id: 'herkunft-aufstieg',
    label: 'Herkunft & Aufstieg',
    color: '#ea580c',
    points: [
      [37.88, -4.78],
      [41.90, 12.48],
...
export default function SenecaLifeMap({ className = '', mapHeightClass = 'h-[420px]' }: SenecaLifeMapProps) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h3 className="font-display text-xl font-bold text-foreground flex items-center gap-2">
              ...
              Senecas Lebensstationen (4 v. Chr. – 65 n. Chr.)
            </h3>
            <p className="text-sm text-muted-foreground mt-1">
              6 Lebensstationen • Von Corduba über Korsika bis zur Macht in Rom
            </p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        {allMarkerPoints.map(point => (
          <CircleMarker
            ...
            radius={9}
            pathOptions={{
              color: point.tone === 'birth' ? '#16a34a' : point.tone === 'exile' ? '#ea580c' : point.tone === 'death' ? '#6b7280' : '#dc2626',
              fillColor: point.tone === 'birth' ? '#4ade80' : point.tone === 'exile' ? '#fb923c' : point.tone === 'death' ? '#9ca3af' : '#f87171',
              fillOpacity: 0.9,
              weight: 3.5,
              opacity: 1,
            }}
        ...
        <div className="flex flex-col gap-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-card/80 border border-border/60 hover:bg-card transition-all shadow-sm">
                <div className="h-4 w-4 rounded-full bg-orange-400 border-[3px] border-orange-600 shadow-md" />
                <span className="font-semibold text-foreground text-xs">Exil</span>
              </div>
              <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-card/80 border border-border/60 hover:bg-card transition-all shadow-sm">
                <div className="h-4 w-4 rounded-full bg-gray-400 border-[3px] border-gray-600 shadow-md" />
                <span className="font-semibold text-foreground text-xs">Tod</span>
              </div>
            </div>
          </div>
        </div>
